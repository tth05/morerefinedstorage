...
class Processing extends Craft {
    ...
    Processing(INetwork network, NBTTagCompound tag) throws CraftingTaskReadException {
        ...
        this.fluidsToUse = CraftingTask.readFluidStackList(tag.getTagList(NBT_FLUIDS_TO_USE, Constants.NBT.TAG_COMPOUND));
        this.itemsToDisplay = CraftingTask.readItemStackList(tag.getTagList(NBT_ITEMS_TO_DISPLAY, Constants.NBT.TAG_COMPOUND));
    }
    ...
    int getNeeded(FluidStack stack) {
        if (fluidsToReceive.get(stack) != null) {
            int needed = fluidsToReceive.get(stack).amount * totalQuantity;
            if (fluidsReceived.get(stack) != null) {
                needed -= fluidsReceived.get(stack).amount;
            }
            return needed;
        }
        return 0;
    ...
    */
        ...
        if (!fluidsToReceive.isEmpty()) {
            for (StackListEntry<FluidStack> stack : fluidsToReceive.getStacks()) {
                if (fluidsReceived.get(stack.getStack()) != null) {
                    if (temp > fluidsReceived.get(stack.getStack()).amount / (fluidsToReceive.get(stack.getStack()).amount)) {
                        temp = fluidsReceived.get(stack.getStack()).amount / (fluidsToReceive.get(stack.getStack()).amount);
                    }
                } else {
                    temp = 0;
                }
    ...
    NBTTagCompound writeToNbt() {
        ...
        tag.setTag(NBT_FLUIDS_TO_USE, CraftingTask.writeFluidStackList(fluidsToUse));
        tag.setTag(NBT_ITEMS_TO_DISPLAY, CraftingTask.writeItemStackList(itemsToDisplay));
