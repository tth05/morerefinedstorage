...
public class CraftingTask implements ICraftingTask {
    ...
    public CraftingTask(INetwork network, NBTTagCompound tag) throws CraftingTaskReadException {
        ...
        this.internalStorage = factoryItem.createFromNbt(null, tag.getCompoundTag(NBT_INTERNAL_STORAGE));
        this.internalFluidStorage = factoryFluid.createFromNbt(null, tag.getCompoundTag(NBT_INTERNAL_FLUID_STORAGE));
        ...
        NBTTagList craftList = tag.getTagList(NBT_CRAFTS, Constants.NBT.TAG_COMPOUND);
        for (int i = 0; i < craftList.tagCount(); ++i) {
            Craft c = Craft.createCraftFromNBT(network, craftList.getCompoundTagAt(i));
    ...
    static NBTTagList writeItemStackList(IStackList<ItemStack> stacks) {
        ...
        for (StackListEntry<ItemStack> entry : stacks.getStacks()) {
            list.appendTag(StackUtils.serializeStackToNbt(entry.getStack()));
        }
    ...
    static NBTTagList writeFluidStackList(IStackList<FluidStack> stacks) {
        ...
        for (StackListEntry<FluidStack> entry : stacks.getStacks()) {
            list.appendTag(entry.getStack().writeToNBT(new NBTTagCompound()));
        }
    ...
    static class PossibleFluidInputs {
        ...
        void sort(IStackList<FluidStack> mutatedStorage, IStackList<FluidStack> results) {
            possibilities.sort((a, b) -> {
                ...
                return (br == null ? 0 : br.amount) - (ar == null ? 0 : ar.amount);
            });
    ...
    private ICraftingTaskError calculateInternal(
        ...
        if (craft instanceof Crafting) {
            ...
            for (FluidStack inputs : pattern.getFluidInputs()) {
                if (inputs == null) {
                    continue;
                }
                ingredientNumber++;

                PossibleFluidInputs possibleInputs = new PossibleFluidInputs(new ArrayList<>(pattern.getFluidInputs()));
                ...
                int remaining = possibleInput.amount * qty;
            ...
            pattern.getFluidOutputs().forEach(x -> fluidResults.add(x, x.amount * qty));
    ...
    private void extractInitial() {
        ...
        if (!toExtractInitialFluids.isEmpty()) {
            ...
            for (StackListEntry<FluidStack> toExtract : toExtractInitialFluids.getStacks()) {
                FluidStack result = network.extractFluid(toExtract.getStack(), toExtract.getStack().amount, Action.PERFORM);

                if (result != null) {
                    internalFluidStorage.insert(toExtract.getStack(), result.amount, Action.PERFORM);
    ...
    private static IStackList<FluidStack> extractFromInternalFluidStorage(Collection<StackListEntry<FluidStack>> stacks, IStorageDisk<FluidStack> storage, Action action) {
        ...
        for (StackListEntry<FluidStack> entry : stacks) {
            FluidStack result = storage.extract(entry.getStack(), entry.getStack().amount, IComparer.COMPARE_NBT, action);
            if (result == null || result.amount != entry.getStack().amount) {
                if (action == Action.PERFORM) {
                    throw new IllegalStateException("The internal crafting inventory reported that " + entry.getStack() + " was available but we got " + result);
                }
                return null;
    ...
    private static boolean insertIntoTank(IFluidHandler dest, Collection<StackListEntry<FluidStack>> toInsert, Action action) {
        for (StackListEntry<FluidStack> entry : toInsert) {
            int filled = dest.fill(entry.getStack(), action != Action.SIMULATE);
            if (filled != entry.getStack().amount) {
                if (action == Action.PERFORM) {
                    LOGGER.warn("Fluid Handler unexpectedly didn't accept all of " + entry.getStack().getLocalizedName() + ", the remainder has been voided!");
                }
                return false;
            }
        }
    ...
    public boolean update() {
        ...
        if (this.crafts.isEmpty()) {
            ...
            for (FluidStack stack : internalFluidStorage.getStacks()) {
                ...
                toPerform.add(() -> internalFluidStorage.extract(stack, stack.amount - remainder.amount, IComparer.COMPARE_NBT, Action.PERFORM));
            }
            ...
            if (craft instanceof Processing) {
                ...
                if (needed > 0) {
                    ...
                    if (!p.isRoot()) {
                        ...
                        internalFluidStorage.insert(remainder, remainder.amount, Action.PERFORM);
                    }
            ...
            if (craft instanceof Crafting ) {
                ...
                for (StackListEntry<ItemStack> put : p.getItemsToDisplay().getStacks()) {
                    if (p.getProcessing() > 0|| p.getState() !=ProcessingState.READY) {
                        ICraftingMonitorElement element = new CraftingMonitorElementItemRender(put.getStack(), 0, 0, put.getStack().getCount() * p.getProcessing(), 0, 0);
                ...
                for (StackListEntry<ItemStack> receive : p.getItemsToReceive().getStacks()) {
                    ...
                    if (count > 0) {
                        elements.add(new CraftingMonitorElementItemRender(receive.getStack(), 0, 0, 0, count, 0),true);
                    }
                }
                for (StackListEntry<FluidStack> put : p.getFluidsToUse().getStacks()) {
                    if (p.getProcessing() > 0|| p.getState() !=ProcessingState.READY) {
                        ICraftingMonitorElement element = new CraftingMonitorElementFluidRender(put.getStack(), 0, 0, put.getStack().amount * p.getProcessing(), 0, 0);
                        if (p.getState() == ProcessingState.MACHINE_DOES_NOT_ACCEPT) {
                ...
                for (StackListEntry<FluidStack> receive : p.getFluidsToReceive().getStacks()) {
                    int count = p.getNeeded(receive.getStack());
                    if (count > 0) {
                        elements.add(new CraftingMonitorElementFluidRender(receive.getStack(), 0, 0, 0, count, 0),true);
                    }
                }
            }
        }
    ...
    public List<ICraftingPreviewElement> getPreviewStacks() {
        ...
        for (StackListEntry<FluidStack> stack : missingFluids.getStacks()) {
            ...
            previewStack.setMissing(true);
            previewStack.addToCraft(stack.getStack().amount);
        ...
        for (StackListEntry<FluidStack> stack : toTakeFluids.getStacks()) {
            ...
            previewStack.addAvailable(stack.getStack().amount);
