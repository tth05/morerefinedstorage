...
public class ContainerDiskManipulator extends ContainerBase {
    public ContainerDiskManipulator(TileDiskManipulator diskManipulator, EntityPlayer player) {
        ...
        for (int i = 0; i < 9; ++i) {
            addSlotToContainer(new SlotFilter(diskManipulator.getNode().getConfig().getItemHandler(), i, 8 + (18 * i), 20).setEnableHandler(() -> diskManipulator.getNode().getConfig().isFilterTypeItem()));
        }

        for (int i = 0; i < 9; ++i) {
            addSlotToContainer(new SlotFilterFluid(diskManipulator.getNode().getConfig().getFluidHandler(), i, 8 + (18 * i), 20).setEnableHandler(() -> diskManipulator.getNode().getConfig().isFilterTypeFluid()));
        }
        ...
        transferManager.addFilterTransfer(player.inventory, diskManipulator.getNode().getConfig().getItemHandler(), diskManipulator.getNode().getConfig().getFluidHandler(), diskManipulator.getNode().getConfig()::getFilterType);
    }
}
