...
- Fixed russian translation for creative range upgrade (bigenergy)
- Fixed tooltip search in grid being incredibly slow
- Fixed machines being able to starting multiple crafting after last change
- Fixed scrolling from grid to inventory not working
- Fixed scrollbar moving when scrolling pattern grid slots
- Fixed Wireless Crafting Grid not using energy when shift crafting
- Fixed crash when a block is broken while the gui is opened by a player
...
- Fixed processing tasks giving items to parents when not allowed
